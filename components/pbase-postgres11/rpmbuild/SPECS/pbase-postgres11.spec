Name: pbase-postgres11
Version: 1.0
Release: 1
Summary: PBase Postgres 11 server rpm
Group: System Environment/Base
License: Apache-2.0
URL: https://pbase-foundation.com
Source0: pbase-postgres11-1.0.tar.gz
BuildArch: noarch
...
parseConfig "CONFIG_DB_HOSTNAME" ".pbase_postgres11[0].default.hostName" "localhost"
parseConfig "CONFIG_DB_PORT"     ".pbase_postgres11[0].default.port" "5432"

parseConfig "CONFIG_DB_STARTSVC" ".pbase_postgres11[0].default.startService" "true"
parseConfig "CONFIG_DB_INSTALL"  ".pbase_postgres11[0].default.install" "true"

parseConfig "CONFIG_DB_NAME"     ".pbase_postgres11[0].default.database[0].name" "app_db"
parseConfig "CONFIG_DB_USER"     ".pbase_postgres11[0].default.database[0].user" "dbappuser"
parseConfig "CONFIG_DB_PSWD"     ".pbase_postgres11[0].default.database[0].password" $RAND_PW_USER
parseConfig "CONFIG_DB_CHARSET"  ".pbase_postgres11[0].default.database[0].characterSet" "UTF8"
parseConfig "CONFIG_CREATEDB"    ".pbase_postgres11[0].default.database[0].grantCreateDatabase" "false"
...
if [[ "${REDHAT_RELEASE_DIGIT}" == "6" ]]; then
  echo "Calling:                 service postgresql-11 initdb"
  /sbin/service postgresql-11 initdb
else
  echo "Calling:                 postgresql-11-setup initdb"
...
/bin/cp "/var/lib/pgsql/11/data/pg_hba.conf" "/var/lib/pgsql/11/data/pg_hba.conf-PREV"
...
echo "Adding login config to:  /var/lib/pgsql/11/data/pg_hba.conf"
echo "                         ${TRUST_ALL_LINE}"
...
POSTGRES_CONF_FILE="/var/lib/pgsql/11/data/postgresql.conf"
...
if [[ "${PORT_NUM_SUFFIX}" != "" ]]; then
  if [[ "$REDHAT_RELEASE_DIGIT" == "6" ]]; then
    ...
    /bin/cp --no-clobber /usr/local/pbase-data/pbase-postgres11/etc-systemd-system/postgresql.service  /etc/systemd/system/
...
# Start and enable postgresql-11 service at boot-time
echo "Starting Service:        postgresql-11"


if [[ "${REDHAT_RELEASE_DIGIT}" == "6" ]]; then
  /sbin/chkconfig postgresql-11 --level 345 on || fail "chkconfig failed to enable postgresql-11 service"
  /sbin/service postgresql-11 start || fail "failed to start postgresql-11 service"
else
  /bin/systemctl daemon-reload
  /bin/systemctl enable postgresql-11.service || fail "systemctl failed to enable postgresql-11 service"
  /bin/systemctl start postgresql-11 || fail "failed to restart postgresql-11 service"
fi


## make a copy of the create-user script
SRC_SCRIPT_DIR="/usr/local/pbase-data/pbase-postgres11/postgres-init"
...
append_bashrc_alias editpostgreshba "vi /var/lib/pgsql/11/data/pg_hba.conf"
...
%files
%defattr(-,root,root,-)
/usr/local/pbase-data/pbase-postgres11/postgres-init/create-dbappuser.sql
/usr/local/pbase-data/pbase-postgres11/etc-systemd-system/postgresql.service
