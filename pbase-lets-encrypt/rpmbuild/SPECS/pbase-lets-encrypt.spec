...
if [[ "${REDHAT_RELEASE_DIGIT}" == "6" ]] || [[ "${REDHAT_RELEASE_DIGIT}" == "8" ]]; then
  CRONJOB_LINE="47 2 * * 2 root /usr/local/bin/certbot-auto renew >> $CRONJOB_LOGFILE"
  ##CRONJOB_LINE="$RAND_MINUTE 2 * * 2 root /usr/local/bin/certbot-auto renew >> $CRONJOB_LOGFILE"
else
  ##CRONJOB_LINE="$RAND_MINUTE 2 * * 2 root /usr/bin/certbot renew >> $CRONJOB_LOGFILE"
  CRONJOB_LINE="47 2 * * 2 root /usr/bin/certbot renew >> $CRONJOB_LOGFILE"
fi
...
if [[ "${REDHAT_RELEASE_DIGIT}" == "6" ]]; then
  ...
  echo "Invoke certbot:          /usr/local/bin/certbot-auto --apache --agree-tos --email ${EMAIL_ADDR}  -d ${THISDOMAINNAME} -d www.${THISDOMAINNAME} $DASH_D_ADDITIONAL_SUBDOMAIN -n"
  echo ""
