...
Source0: pbase-preconfig-postgres-nextcloud-1.0.tar.gz
BuildArch: noarch
...
echo "PBase Postgres create config preset user and DB name for use by pbase-nextcloud, add Postgres PHP dependencies"
...
MODULE_CONFIG_DIR="/usr/local/pbase-data/admin-only/module-config.d"
MODULE_SAMPLES_DIR="/usr/local/pbase-data/pbase-preconfig-postgres-nextcloud/module-config-samples"
...
echo "Postgres user and DB name for use by pbase-nextcloud rpm"
echo "Setting database 'password' field in ${DB_CONFIG_FILENAME}"
...
echo ""
echo "Postgres, SMTP and Let's Encrypt module config files for Nextcloud added."
