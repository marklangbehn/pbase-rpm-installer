Name: pbase-preconfig-terraform
Version: 1.0
Release: 0
...
Source0: pbase-preconfig-terraform-1.0.tar.gz
BuildArch: noarch
BuildRoot: %{_tmppath}/%{name}-buildroot

Provides: pbase-preconfig-terraform
...
echo "PBase Terraform pre-config file create"

MODULE_CONFIG_DIR="/usr/local/pbase-data/admin-only/module-config.d"
MODULE_SAMPLES_DIR="/usr/local/pbase-data/pbase-preconfig-terraform/module-config-samples"
CONFIG_FILENAME="pbase_terraform.json"

echo "Terraform config:        ${MODULE_CONFIG_DIR}/${CONFIG_FILENAME}"
...
echo "Next step - install Terraform with:"
echo ""
echo "  yum -y install pbase-terraform"
echo ""

%files
%defattr(600,root,root,700)
/usr/local/pbase-data/pbase-preconfig-terraform/module-config-samples/pbase_terraform.json
